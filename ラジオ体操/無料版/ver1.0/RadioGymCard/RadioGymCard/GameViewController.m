...
- (void)viewDidLoad
{
    ...
    nadView_ =[[NADView alloc] initWithFrame:
               CGRectMake(0,430, NAD_ADVIEW_SIZE_320x50.width, NAD_ADVIEW_SIZE_320x50.height )]; //NADView の作成
    //[nadView_ setNendID:@"a6eca9dd074372c898dd1df549301f277c53f2b9"spotID:@"3172"];            //テスト用  apiKey, spotId.
    [nadView_ setNendID:@"35b307f850476a563a37c690b895be2f148a2daf"spotID:@"15483"];             //本番用  set apiKey, spotId.
...
- (IBAction)pushStamp:(id)sender {    
    ...
    //アニメーション
    handImageView.center = CGPointMake(x,y-40);    
    [UIView beginAnimations:nil context:nil];
    [UIView setAnimationDuration:0.1];
    handImageView.center = CGPointMake(x , y);
    ...
    if(buttonTag == monthEndDay){
        ...
        //カレンダーを左にスライド
        [self slideCalender:-160 yposition:257];
...
-(void)clearCalender:(NSTimer *)timer{
    ...
    //画面右側にワープ
    calender.center = CGPointMake(480,257);
    //左にスライドするアニメーション
    [self slideCalender:160 yposition:257];
...
-(void)gameOver:(NSInteger)message {
    ...
    gameOverView.center = CGPointMake(160,215);
    [UIView commitAnimations];
    ...
    if(modeFlg == YEARMODE){//365日モード
        ...
        GKScore *scoreReporter = [[GKScore alloc] initWithCategory:@"RadioGymFreeTimeAttackSummer"];
        NSLog(@"リーダーボードに送信する値：%d",(int)(elapsedTime*100));
